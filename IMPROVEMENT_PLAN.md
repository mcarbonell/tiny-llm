...
- `requirements.txt` esta sobrecargado y tiene versiones duplicadas/conflictivas.
...
1. Limpiar dependencias y reproducibilidad
...
- Revisar el test de roundtrip en [`tests/test_model.py`](tests/test_model.py) para que compruebe el comportamiento correcto, no una igualdad demasiado estricta.
...
1. Mantener este plan actualizado
...
- El test `test_tokenizer_roundtrip` falla porque `decode()` devuelve un espacio inicial extra.
...
3. Corregir o rehacer `test_tokenizer_roundtrip`.
