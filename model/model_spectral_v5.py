"""
model_spectral_v5.py — SpectralThinker v5 (JPEG-LLM / Temporal KV Cache Compression)

Cambios vs v4:
  - Implementación de la teoría V65 "Text JPEG": Compresión temporal del KV Cache.
  - La dimensión de la secuencia (seq_len) se comprime usando 1D-DCT a 'k_seq_len'.
  - Durante el entrenamiento (cuando seq_len > 1 y past_kv no se provee),
    no se afecta la calidad de aprendizaje actual, pero la arquitectura base
    ahora soporta la idea de "recuerdo borroso/estructural" del pasado.
"""
...
# =============================================================================
# 1. SpectralArgs (Añadido k_seq_len)
# =============================================================================
...
class SpectralArgs:
    ...
    # Compresión Frecuencial Interna (Pesos)
    k_dim_attn: int = 64
    k_dim_ffn: int = 64
    k_hidden_ffn: int = 128
    
    # NUEVO: Compresión Frecuencial Externa (Temporal KV Cache)
    k_seq_len: int = 64   # Cuántas frecuencias guardar de la secuencia pasada
...
# =============================================================================
# 4. Componentes estándar (RMSNorm, RoPE)
# =============================================================================
...
# =============================================================================
# 5. Temporal DCT Cache Attention (JPEG-LLM)
# =============================================================================

class SpectralAttentionV5(nn.Module):
    def __init__(self, args: SpectralArgs):
        ...
        # Temporal compression parameter
    ...
    def _compress_temporal(self, tensor: torch.Tensor, device) -> torch.Tensor:
        """
        Comprime la dimensión temporal (seq_len) guardando solo k_seq_len coeficientes DCT.
        Tensor shape: (bsz, seq_len, heads, head_dim) -> (bsz, k_seq_len, heads, head_dim)
        """
        bsz, seq_len, heads, h_dim = tensor.shape
        if seq_len <= self.k_seq_len:
            return tensor # No hace falta comprimir aún
        
        # Generamos/recuperamos base (k_seq_len x seq_len)
        D_seq = _get_dct_rows(seq_len, self.k_seq_len).to(device)
        
        # Transponemos para que seq_len quede al final para la multiplicación
        # tensor: (bsz, heads, h_dim, seq_len)
        tensor_t = tensor.permute(0, 2, 3, 1)
        
        # Multiplicación mat: (bsz, heads, h_dim, k_seq_len)
        compressed = tensor_t @ D_seq.t()
        
        # Restauramos forma: (bsz, k_seq_len, heads, h_dim)
        return compressed.permute(0, 3, 1, 2)
        
    def _decompress_temporal(self, compressed_tensor: torch.Tensor, target_seq_len: int, device) -> torch.Tensor:
        """
        Descomprime de vuelta a target_seq_len antes de hacer attention con el nuevo query.
        Tensor shape: (bsz, k_seq_len, heads, head_dim) -> (bsz, target_seq_len, heads, head_dim)
        """
        bsz, current_len, heads, h_dim = compressed_tensor.shape
        if current_len == target_seq_len:
            return compressed_tensor # Estaba intacto
        
        # Base completa necesaria para la reconstrucción inversa
        D_seq = _get_dct_rows(target_seq_len, self.k_seq_len).to(device) # (k_seq_len x target_seq_len)
        
        compressed_t = compressed_tensor.permute(0, 2, 3, 1) # (bsz, heads, h_dim, k_seq_len)
        
        # Reconstrucción: X_approx = X_freq @ D
        decompressed = compressed_t @ D_seq # (bsz, heads, h_dim, target_seq_len)
        
        return decompressed.permute(0, 3, 1, 2)
    ...
    def forward(self, x, freqs_cis, mask, past_key_value=None):
        ...
        # En inferencia (generación token a token), seqlen = 1
        if past_key_value is not None:
            pk_compressed, pv_compressed, past_seq_len = past_key_value
            
            # 1. Descomprimir el pasado al tamaño real + 1
            pk = self._decompress_temporal(pk_compressed, past_seq_len, x.device)
            pv = self._decompress_temporal(pv_compressed, past_seq_len, x.device)
            
            # 2. Concatenar el nuevo token actual (tamaño temporal = past_seq_len + 1)
            xk_full = torch.cat([pk, xk], dim=1)
            xv_full = torch.cat([pv, xv], dim=1)
            
            # 3. Comprimir de nuevo para el siguiente ciclo
            new_past_len = past_seq_len + seqlen
            xk_to_save = self._compress_temporal(xk_full, x.device)
            xv_to_save = self._compress_temporal(xv_full, x.device)
            new_kv = (xk_to_save, xv_to_save, new_past_len)
            
            # Hacemos attention con las secuencias completas reconstruidas
            xq_t, xk_t, xv_t = xq.transpose(1,2), xk_full.transpose(1,2), xv_full.transpose(1,2)
            out = F.scaled_dot_product_attention(xq_t, xk_t, xv_t, attn_mask=None, dropout_p=0.0, is_causal=False)
        else:
            # Entrenamiento o primer token: calculamos sobre toda la ventana actual
            # Guardamos el caché comprimido para la posible fase de inferencia posterior
            xk_to_save = self._compress_temporal(xk, x.device)
            xv_to_save = self._compress_temporal(xv, x.device)
            new_kv = (xk_to_save, xv_to_save, seqlen)
            
            # NOISE-INJECTION: Forzamos a la red a entrenar con la versión "borrosa" del KV
            # para que sea robusta en inferencia.
            xk_approx = self._decompress_temporal(xk_to_save, seqlen, x.device)
...
class SpectralTransformerBlock(nn.Module):
    def __init__(self, args: SpectralArgs):
        super().__init__()
        self.attention      = SpectralAttentionV5(args)
