"""
model_spectral_v4.py — SpectralThinker v4 (Midi Scale ready, NO weight tying)

Cambios vs v3:
  - Se eliminó el weight tying (separación de tok_embeddings y output)
    para mejorar el rendimiento en modelos de escala pequeña (Nano/Midi).
  - Matrix-free forward conservado.
  - Walsh truncado eficiente conservado.
"""
...
class SpectralThinker(nn.Module):
    def __init__(self, args: SpectralArgs):
        ...
        # Inicialización de pesos sin weight tying
        nn.init.normal_(self.output.weight, mean=0.0, std=1.0 / math.sqrt(args.dim))
        # No se realiza: self.output.weight = self.tok_embeddings.weight
